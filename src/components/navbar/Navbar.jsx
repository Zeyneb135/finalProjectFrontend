...
const Navbar = () => {
    ...
    const handleRedirectToDashboard = () => {
        if (!user) {
            ...
            // Parol sorğusu əlavə etmək
            const enteredPassword = prompt("Please enter the password to access the Dashboard:");
            
            const correctPassword = "ziko135"; // Burada doğru parolu daxil edin

            if (enteredPassword === correctPassword) {
                navigate('/admin'); // Parol doğru olduğu halda admin panelinə yönləndir
            } else {
    ...
    // `user` və `user.otherDetails` yoxlanması
    const username = user && user.otherDetails ? user.otherDetails.username : 'User';  // 'User' defolt dəyəri
